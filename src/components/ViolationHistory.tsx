...
import { History, Search, RefreshCw, X, ShieldCheck, ShieldAlert } from 'lucide-react';
...
export default function ViolationHistory({
	...
	const [startD, setStartD] = useState(filters.start_date ? filters.start_date.split('T')[0] : '');
	const [endD, setEndD] = useState(filters.end_date ? filters.end_date.split('T')[0] : '');
	...
	const handleApplyFilters = (e: React.FormEvent) => {
		...
		setFilters({
			...
			end_date: endD ? new Date(endD + 'T23:59:59.999Z').toISOString() : undefined,
	...
	return (
		<div className="space-y-6">
			...
			<Card className="border-zinc-200 dark:border-zinc-800 shadow-sm">
				...
				<CardContent>
					{isLoading ? (
						<div className="py-20 flex justify-center text-zinc-400">Loading violations...</div>
					) : violations.length === 0 ? (
								...
								<tbody className="divide-y divide-zinc-100 dark:divide-zinc-850">
									{violations.map((v) => {
										...
										return (
											...
											>
												<td className="p-3 font-mono text-[10px] text-zinc-400">{v.id}</td>
												<td className="p-3 font-semibold text-zinc-900 dark:text-zinc-100">{v.vehicle_number}</td>
												<td className="p-3 text-zinc-700 dark:text-zinc-300">{v.geofence_name}</td>
												<td className="p-3">
													<Badge
														variant="outline"
													...
													>
														{isEntry ? <ShieldCheck className="w-3.5 h-3.5" /> : <ShieldAlert className="w-3.5 h-3.5" />}
														{v.event_type}
													</Badge>
												</td>
