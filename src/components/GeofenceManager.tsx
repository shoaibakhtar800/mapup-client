...
export default function GeofenceManager({
	...
	const handleStartDrawing = () => {
		...
		setDrawPoints([]);
		toast.info('Drawing mode activated! Click on the map to define the polygon boundaries.');
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (drawPoints.length < 3) {
			toast.error('Polygon requires at least 3 unique points to define a boundary.');
			return;
		}

		// Geofence specs require first and last coordinates to be identical (closed polygon)
		const closedCoordinates: [number, number][] = [...drawPoints, drawPoints[0]];
		...
		<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				...
				<CardContent>
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						<div className="space-y-2 border-t pt-3 mt-4">
							<Label className="block mb-2 font-medium">Boundary coordinates</Label>
							{!isDrawingGeofence ? (
								<Button
									type="button"
									variant="outline"
								...
								<div className="space-y-3">
									...
									<div className="text-[11px] bg-zinc-50 dark:bg-zinc-900 border border-zinc-150 p-2 rounded max-h-24 overflow-y-auto font-mono text-zinc-500">
										{drawPoints.length === 0 ? (
											<span className="text-zinc-400 italic">No points drawn yet</span>
										) : (
											drawPoints.map((pt, i) => (
												<div key={i}>
			...
			<Card className="md:col-span-2 border-zinc-200 dark:border-zinc-800">
				<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
					<div>
						<CardTitle className="text-zinc-950 dark:text-zinc-50">Active Geofences</CardTitle>
						<CardDescription>
				...
				<CardContent>
					{isLoading ? (
						<div className="py-12 flex justify-center text-zinc-400">Loading geofences...</div>
						...
						<div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
							{filteredGeofences.map((geo) => {
								...
								const catColors: Record<string, string> = {
									delivery_zone: 'bg-blue-50 text-blue-700 dark:bg-blue-950/20 dark:text-blue-400 border-blue-200/50',
									restricted_zone: 'bg-red-50 text-red-700 dark:bg-red-950/20 dark:text-red-400 border-red-200/50',
									toll_zone: 'bg-purple-50 text-purple-700 dark:bg-purple-950/20 dark:text-purple-400 border-purple-200/50',
									customer_area: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400 border-emerald-200/50',
								};
								return (
									<div
										key={geo.id}
									...
									>
										<div className="space-y-1">
											...
											<div className="text-[10px] text-zinc-400">
												ID: <span className="font-mono">{geo.id}</span> &bull; {geo.coordinates.length - 1} vertices
											</div>
										</div>
									</div>
								);
