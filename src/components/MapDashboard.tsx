...
const getVehicleIcon = (vehicleType: string, isRestricted: boolean) => {
	...
	let path = `M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2`; // Default Truck-ish
	if (vehicleType === 'car') {
		path = `M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3C13 6.8 11.8 6 10.5 6H5C3.3 6 2 7.3 2 9v7c0 .6.4 1 1 1h2`;
	}
		...
		<div className="relative w-full h-full min-h-[400px] overflow-hidden">
			...
			>
						...
						>
							<Popup>
								<div className="p-1 text-zinc-900 font-sans">
									<h4 className="font-semibold text-sm m-0 text-zinc-950">{geo.name}</h4>
									<p className="text-xs text-zinc-500 my-1">{geo.description || 'No description'}</p>
				...
				{Object.values(vehicleLocations).map((loc) => {
					...
					return (
						<Marker
							key={loc.vehicle_id}
							position={[loc.current_location.latitude, loc.current_location.longitude]}
							icon={getVehicleIcon(vehicle?.vehicle_type || 'truck', isInRestricted)}
						>
							<Popup>
								<div className="p-1 text-zinc-900 font-sans leading-5">
									...
									<div className="text-xs text-zinc-600 mt-1">
										...
										<p><strong>Type:</strong> <span className="capitalize">{vehicle?.vehicle_type}</span></p>
										<p><strong>Last Updated:</strong> {new Date(loc.current_location.timestamp).toLocaleTimeString()}</p>
									</div>

									{loc.current_geofences.length > 0 && (
										<div className="mt-2 border-t pt-1">
											<p className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Current Zones:</p>
											<div className="flex flex-wrap gap-1">
												{loc.current_geofences.map((cg) => {
													const isRestrictedZone = cg.category === 'restricted_zone';
													return (
														<span
															key={cg.geofence_id}
			...
			{isSettingLocation && selectedVehicleId && (
				<div className="absolute bottom-4 left-4 bg-zinc-900/90 text-white text-xs px-3 py-2 rounded-lg backdrop-blur-sm shadow-lg pointer-events-none z-[1000] border border-zinc-700/50 flex items-center gap-2">
					...
					Click anywhere on the map to set the new location for the active vehicle
				</div>
			)}
			{isDrawingGeofence && (
				<div className="absolute bottom-4 left-4 bg-zinc-900/90 text-white text-xs px-3 py-2 rounded-lg backdrop-blur-sm shadow-lg pointer-events-none z-[1000] border border-zinc-700/50 flex flex-col gap-1">
					...
					<div className="text-zinc-300 text-[10px]">
						Click to define vertices. Use form buttons to undo, clear, or complete.
					</div>
				</div>
			)}
		</div>
