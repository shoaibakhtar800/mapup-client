              ...
              <Select value={eventType} onValueChange={setEventType}>
                ...
                <SelectContent>
                  <SelectItem value="entry">
                    Entry (Vehicle enters area)
                  </SelectItem>
                  <SelectItem value="exit">
                    Exit (Vehicle leaves area)
                  </SelectItem>
                  <SelectItem value="both">
                    Both (Enters or leaves area)
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="py-12 text-center text-zinc-400 italic border border-dashed rounded-lg border-zinc-200 dark:border-zinc-800">
              No custom alert rules configured. Create one above to map custom
              notifications!
            </div>
          ) : (
            <div className="space-y-2 max-h-75 overflow-y-auto pr-1">
              {alerts.map((al) => {
                ...
                const isGlobal =
                  !al.vehicle_id || al.vehicle_number === "All Vehicles";
                const isRestricted =
                  geofences.find((g) => g.id === al.geofence_id)?.category ===
                  "restricted_zone";
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <div className="text-[10px] text-zinc-400">
                        Rule ID:{" "}
                        <span className="font-mono">{al.alert_id}</span> &bull;
                        Configured:{" "}
